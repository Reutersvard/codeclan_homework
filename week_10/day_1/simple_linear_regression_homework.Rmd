...
project %>% 
  ...
  geom_point() +
  # This is just to visualise the regression for the questions below.
...
```
This is a high R value, it indicates that the two variables may be correlated.
...
summary(model)
```
The gradient is 1.2235, i.e. for every unit measured in `actual_length` the `estimated_length` is 1.2235. $R^2$ takes values between 0 and 1, so in this case it is quite high, suggesting a strong correlation.
...
The residuals and standardised residuals plots show that variance is quite constant until we get to higher values, probably due to the outlier (number 5).
...
sampled %>% 
  visualise(bins = 30) +
  shade_ci(endpoints = get_ci(sampled, level = 0.95))
```
The gradient value falls around 1.2 in a bootstrapped distribution. All of the coefficient values in this resampled distribution are greater than 0, the p-value is 1.911284e-12, very close to 0, so this indicates we can reject $H_0$ (that the coefficient = 0) and conclude that the model is a good fit for our data, i.e. $actual\_length = 1.2235\ estimated\_length + 1.4164$.
...
From the diagnostic plots above, 5, 18 and 23 may be "problematic" (big variance from a linear model)
...
```{r}
tidy(lm(actual_length ~ estimated_length, project[2:nrow(project), ]))
```
```{r}
project %>% 
  add_predictions(model) %>% 
  rename("pred_all_points" = "pred") %>% 
  add_predictions(lm(actual_length ~ estimated_length, project[2:nrow(project), ])) %>% 
  ggplot() +
  geom_line(aes(estimated_length, pred_all_points)) +
  geom_line(aes(estimated_length, pred), col = "red") 
```

It does not affect the model much.
...
project %>% 
  add_predictions(model) %>% 
  rename("pred_all_points" = "pred") %>% 
  add_predictions(lm(actual_length ~ estimated_length, project[c(1:4, 6:nrow(project)), ])) %>% 
  ggplot() +
  geom_line(aes(estimated_length, pred_all_points)) +
  geom_line(aes(estimated_length, pred), col = "red") 
```

On this occasion the gradient and intercept of the model change a lot.
