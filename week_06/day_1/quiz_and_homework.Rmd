...
**Question**  
Calculate the conditional probabilities $P(\textrm{survived} \mid \textrm{crew})$ and $P(\textrm{survived} \mid \textrm{passenger})$ to settle this dispute.
...
```{r}
titanic <- titanic %>% 
  mutate(crew = if_else(str_detect(class, "[1-3]"), F, T))

#Prob of surviving if crew
p_survived_crew <- titanic %>% 
  filter(crew == T & survived == "yes") %>% 
  nrow() / nrow(filter(titanic, crew == T))

#Prob of surviving if passenger
p_survived_passanger <- titanic %>% 
  filter(crew == F & survived == "yes") %>% 
  nrow() / nrow(filter(titanic, crew == F))
```
...
```
  ...
  * what assumptions do we apply in answering part ii?
  ...
  That the four events (i.e. dice rolls) are independent
...
```{r}
perms <- as_tibble(permutations(5, 5, repeats.allowed = F))
perms %>% 
  filter(V1 == 5,
         V2 == 4,
         V3 == 3,
...
```{r}
1 / as_tibble(permutations(6, 6, repeats.allowed = T)) %>% 
  nrow()
```
  
  * what is the probability I see one each of the numbers 6, 5, 4, 3, 2 and 1 on the dice regardless of order?
```{r}
six_dice <- as_tibble(permutations(6, 6, repeats.allowed = T))

six_dice %>% 
  filter(isin(., c(6, 5, 4, 3, 2, 1), ordered = F)) %>% 
  nrow() / nrow(six_dice)
```
