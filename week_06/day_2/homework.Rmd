...
phones %>% 
  ...
  geom_histogram(aes(y = ..density..), col = "white", bins = 30) +
   stat_function(fun = dnorm,
                 args = list(mean = phones_stats$av_rating,
                             sd = phones_stats$sd_rating),
                 col = "red")
```
...
phones %>% 
  filter(brand == "Samsung",
         rating <= phones_stats$av_rating + phones_stats$sd_rating,
         rating >= phones_stats$av_rating - phones_stats$sd_rating ) %>% 
  nrow() / nrow(filter(phones, brand == "Samsung"))

phones %>% 
  filter(brand == "Samsung",
         rating < phones_stats$av_rating + 2 * phones_stats$sd_rating) %>% 
  nrow() / nrow(filter(phones, brand == "Samsung"))

phones %>% 
  filter(brand == "Samsung",
         rating < phones_stats$av_rating + 3 * phones_stats$sd_rating) %>% 
  nrow() / nrow(filter(phones, brand == "Samsung"))
```

```{r}
# For a normal distribution it would be:
pnorm(q = 1)
pnorm(q = 2)
pnorm(q = 3)
```
