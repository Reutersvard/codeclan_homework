...
```{r}
salaries %>% 
  filter(age > 50 & current_salary > 20E3)
```
...
```{r}
salaries %>% 
  summarise(max_current_salary = max(current_salary),
            min_current_salary = min(current_salary))
```
...
```{r}
beer %>% 
  filter(!str_detect(carbohydrates, "[g]") | is.na(carbohydrates))
```
